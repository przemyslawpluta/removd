...
const supportOptions = {
    channels: (channel) => {
        return check(api.chennelSupport, channel);
    },
    detect: (type) => {
        return check(api.typeSupport, type);
    },
    size: (size) => {
        return check(api.sizeSupport, size);
    },
    format: (format) => {
        return check(api.formatSupport, format);
    },
    background: (color) => {
...
function supportedFiles(ext, extra = []) {
    return check([...api.filesSupport, ...extra], ext);
}
...
function getImageDetails(dim) {
    ...
    let size = api.sizeSupport[0].name;

    if (mpx > api.sizeSupport[0].mpx) {
        size = api.sizeSupport[1].name;
    }

    if (mpx > api.sizeSupport[1].mpx) {
        size = api.sizeSupport[2].name;
    }

    if (mpx > api.sizeSupport[2].mpx) {
        size = api.sizeSupport[3].name;
    }

    const max = api.sizeSupport.filter(item => item.name === size).shift().dim.split('×');
